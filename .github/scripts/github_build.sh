...
if [[ -f "$_root_dir/epoch_job_start.txt" ]]; then
  ...
  # GitHub's hard time limit is 6 h per job, we want to spare 30 min for steps after the build, 
  # To get the remaining time for building we subtract 360*60s - 30*60s - (epoch_now - epoch_job_start)
  _remaining_time=$(( 360*60 - 30*60 - $(date +%s) + epoch_job_start ))
fi
